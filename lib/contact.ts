...
export function validateContact(values: ContactValues, locale: Locale): ContactErrors { const es = locale === "es"; const errors: ContactErrors = {}; if (!values.name.trim()) errors.name = es ? "Ingresá tu nombre." : "Enter your name."; if (!values.problem.trim()) errors.problem = es ? "Contá cuál es el problema." : "Tell me about the problem."; if (!values.email.trim() && !values.whatsapp.trim()) errors.contact = es ? "Dejá un email o WhatsApp para responderte." : "Leave an email or WhatsApp number so I can reply."; if (!values.consent) errors.consent = es ? "Necesito tu consentimiento para usar estos datos y responderte." : "I need your consent to use these details and reply."; return errors; }
